...
const Notifications = () => {
  ...
  }, [activeTab, pagination.page, sentPagination.page]);
  ...
  const toggleSelectAll = () => {
    const list = activeTab === 'received' ? notifications : sentNotifications;
    if (selectedIds.length === list.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(list.map(n => n._id));
    }
  };

  const handleDelete = async (ids = selectedIds) => {
    ...
    try {
      if (activeTab === 'received') {
        ...
        await api.delete('/admins/notifications/sent/delete', { data: { ids } });
        fetchSentNotifications();
      }
  ...
  const fetchSentNotifications = async () => {
    try {
      setLoading(true);
      const response = await api.get('/admins/notifications/sent', {
        params: {
      ...
      if (response.data.success) {
        setSentNotifications(response.data.data.notifications || []);
        setSentPagination({
          page: response.data.data.pagination?.page || 1,
          totalPages: response.data.data.pagination?.totalPages || 1,
          total: response.data.data.pagination?.total || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setDriverSearchTerm('');
        setShowModal(false);
        // If we are on sent tab, refresh it, otherwise switch to it or stay
        if (activeTab === 'sent') {
          fetchSentNotifications();
        }
      } else {
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center space-x-2 bg-gray-100 p-1 rounded-lg">
            <button
              onClick={() => { setActiveTab('received'); setSelectedIds([]); }}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'received' ? 'bg-white text-[#0B2C4D] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Received
            </button>
            <button
              onClick={() => { setActiveTab('sent'); setSelectedIds([]); }}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'sent' ? 'bg-white text-[#0B2C4D] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Sent
            </button>
          </div>
          ...
          <div className="px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <input
                  type="checkbox"
                  checked={(activeTab === 'received' ? notifications : sentNotifications).length > 0 && selectedIds.length === (activeTab === 'received' ? notifications : sentNotifications).length}
                ...
                <h2 className="text-sm font-bold text-gray-900 flex items-center">
                  ...
                  {activeTab === 'received' && unreadCount > 0 && <span className="text-xs font-normal text-gray-500 ml-1">({unreadCount} unread)</span>}
                </h2>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-gray-500 font-medium text-xs">Loading {activeTab} notifications...</div>
              </div>
            </div>
          ) : (activeTab === 'received' ? notifications : sentNotifications).length === 0 ? (
            <div className="text-center py-8">
              <span className="material-icons-outlined text-4xl text-gray-300 mb-2 block">notifications_none</span>
              <p className="text-gray-500 font-medium text-sm">No {activeTab} notifications found</p>
            </div>
          ) : (
            <>
              <div className="divide-y divide-gray-200 contents-list">
                {(activeTab === 'received' ? notifications : sentNotifications).map((notification, index) => (
                  <div
                    key={notification._id}
                    className={`p-3 hover:bg-gray-50 transition-colors duration-200 animate-fade-in ${activeTab === 'received' && !notification.isRead ? 'bg-blue-50/50' : ''
                      }`}
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${activeTab === 'received' ? getNotificationColor(notification.type) : 'bg-green-100 text-green-700'
                          }`}
                      >
                        <span className="material-icons-outlined text-base">
                          {activeTab === 'received' ? getNotificationIcon(notification.type) : 'send'}
                        </span>
                      </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-0.5">
                              ...
                              {activeTab === 'received' && !notification.isRead && (
                              ...
                              {activeTab === 'sent' && (
                                <span className="px-1.5 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wide bg-blue-100 text-blue-800">
              ...
              {/* Pagination */}
              {(activeTab === 'received' ? pagination : sentPagination).totalPages > 1 && (
                <div className="px-4 py-3 border-t border-gray-200 bg-gray-50">
                  <div className="flex items-center justify-between">
                    <div className="text-[10px] sm:text-xs text-gray-600">
                      Page {(activeTab === 'received' ? pagination : sentPagination).page} of {(activeTab === 'received' ? pagination : sentPagination).totalPages}
                    </div>
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => activeTab === 'received' ? setPagination((prev) => ({ ...prev, page: prev.page - 1 })) : setSentPagination((prev) => ({ ...prev, page: prev.page - 1 }))}
                        disabled={(activeTab === 'received' ? pagination : sentPagination).page === 1}
                        className={`px-3 py-1 rounded-lg font-medium text-xs ${(activeTab === 'received' ? pagination : sentPagination).page === 1
                      ...
                      <button
                        onClick={() => activeTab === 'received' ? setPagination((prev) => ({ ...prev, page: prev.page + 1 })) : setSentPagination((prev) => ({ ...prev, page: prev.page + 1 }))}
                        disabled={(activeTab === 'received' ? pagination : sentPagination).page === (activeTab === 'received' ? pagination : sentPagination).totalPages}
                        className={`px-3 py-1 rounded-lg font-medium text-xs ${(activeTab === 'received' ? pagination : sentPagination).page === (activeTab === 'received' ? pagination : sentPagination).totalPages
                          ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                          : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300'
                          }`}
                      >
